...
//                Gate: |wsProgress| in the correct direction >= liftGateThreshold.
...
//   _currentHalf == 0 → LEFT shift (body over left leg, RIGHT foot swings)
//   _currentHalf == 1 → RIGHT shift (body over right leg, LEFT foot swings)
//
...
// joints. This is expected and acceptable in Phase 1. Phase 4 merges these.
// =============================================================================
...
void GaitController::start(GaitMode mode) {
    ...
    // RUNNING, SINGLE_STEP, WSHIFT_ONLY all start from half=0 (right foot swings).
    _currentHalf  = 0;
    _halfPhase    = 0.0f;
    _subState     = StepSubState::WAIT_SHIFT;
    _lastShiftDir = 0;
...
// ---------------------------------------------------------------------------
//  _submitSwingLift  —  SOURCE_GAIT hip extension + knee flexion for one leg.
//  frac=0 → grounded (no command); frac=1 → peak lift.
// ---------------------------------------------------------------------------
void GaitController::_submitSwingLift(uint8_t hipCh, uint8_t kneeCh, float frac) {
    if (!_mm) return;
    // Negative hip angle = extend (lifts thigh up away from neutral).
    // Positive knee angle = flex (bends knee for ground clearance).
...
void GaitController::update(float dt_s, const IMUState& imuState) {
    ...
    // Using a signed check prevents a residual right-shift progress from
    // passing the gate immediately at the start of the next left-shift.
    if (_currentHalf == 0) {
    ...
    // Update display phase and sub-state for telemetry.
    _state.subState = _subState;
    _state.phase    = _currentHalf * 0.5f + _halfPhase * 0.5f;

    // ── WSHIFT_ONLY mode ──────────────────────────────────────────────────────
    // Alternates CoM shift with no foot lift. Use to validate shift direction
    // and ramp completion before enabling stepping.
    ...
    // ── RUNNING / SINGLE_STEP: weight-shift-gated stepping ───────────────────
